...
public class PanelMain extends CenterPanel {
	...
	public PanelMain() {
		...
		//		b1_create.setFont(FrameMain.getTextFont());
		b1_create.setLocation(50, 150);
		b1_create.addActionListener(
				e -> ((FrameMain) Controller.getInstance().getGUIController()).panelCreateFunction());
		add(b1_create);
		
		b2_load = new CustomButtonMedium(Strings.getStringAsHTML("PanelMain.button_load")); //$NON-NLS-1$
		b2_load.setLocation(350, 150);
		b2_load.addActionListener(e -> ((FrameMain) Controller.getInstance().getGUIController()).panelLoadFunction());
		add(b2_load);
		...
		b3_intersection.addActionListener(
				e -> ((FrameMain) Controller.getInstance().getGUIController()).panelIntersection_FunctionSelection());
		add(b3_intersection);
